...
/* Free-spin mode (moon_spin_mode) lives in the C++ ConfigStorage. This plain-C
 * file cannot call it directly, so the .ino provides this shim. */
extern uint8_t moon_cfg_spin_mode(void);
...
void moon_drag_end(void)
{
    ...
    uint8_t spin_mode = moon_cfg_spin_mode();
