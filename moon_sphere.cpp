 ...
 * @brief Render the Moon page as a textured, sub-solar-lit 3D sphere using the
 *        tgx software renderer (https://github.com/vindar/tgx).
 *
 * Ported from the ESP-IDF NINA project to this ESP32-P4 Arduino project.
 * Adaptations vs. the NINA source:
 *  - Texture comes from the vendored byte array MOON_EQUIRECT_JPG (moon_equirect_data.h)
 *    decoded with stb_image, instead of a linker-embedded binary asset.
 *  - Orientation config comes from the global `configStorage` (config_storage.h)
 *    instead of app_config_get().
 *  - Logging uses the project LOG_*_F macros (logging.h) instead of ESP_LOG*.
 *  - New: moon_sphere_set_disk_scale() controls the disk size within the frame so
 *    the starfield/glow background can fill out to the screen edge.
 *
 * ESP32-P4 notes:
...
/* Project's vendored stb_image (implementation lives in stb_image.c, already
 * compiled as C). Include the header with C linkage so this C++ TU does not
 * name-mangle stbi_load_from_memory / stbi_image_free. Do NOT define
 * STB_IMAGE_IMPLEMENTATION here. */
extern "C" {
#include "stb_image.h"
}

/* Vendored embedded lunar texture byte array (moon_equirect.jpg). Declares
 * const unsigned char MOON_EQUIRECT_JPG[] and const unsigned int
 * MOON_EQUIRECT_JPG_LEN. Included from exactly one TU (this one). */
#include "moon_equirect_data.h"

/* Runtime orientation config (moon flip U/V, roll/yaw/pitch offsets, north-up).
 * This is the global ConfigStorage instance declared in config_storage.h. */
#include "config_storage.h"

/* Project logging macros (LOG_INFO_F / LOG_WARNING_F / LOG_ERROR_F). */
#include "logging.h"

using namespace tgx;

/* ----------------------------------------------------------------------------
 * Disk scale (NEW vs. NINA). 1.0 = disk fills the frame edge-to-edge (identical
 * to NINA's fixed ORTHO_R = 1.08). <1.0 shrinks the disk within the frame so the
 * starfield/glow background fills out to the screen edge. Larger ORTHO_R -> a
 * smaller disk, so ORTHO_R is derived as 1.08f / s_disk_scale. Clamped in the
 * setter. Applied to the next render. */
static float s_disk_scale = 1.0f;

void moon_sphere_set_disk_scale(float scale)
{
...
/* ----------------------------------------------------------------------------
 * Orientation tunables (compile-time fallbacks; the live path reads config).
 * Defaults yield the STANDARD near-side naked-eye view: north up, selenographic
 * EAST (Mare Crisium, +lon) on the RIGHT, lunar south (Tycho) at the BOTTOM.
 *
 * The texture U/V flips and the roll/yaw/pitch trims are RUNTIME config
 * (configStorage.getMoonFlipU/V, getMoonRollOffset/getMoonYawOffset/
 * getMoonPitchOffset), all defaulting to 0, so the default behavior matches this
 * base orientation. Set MOON_DEBUG_GEOCENTRIC to 1 to force roll=0 AND
 * libration=0 (north-up base) for direct comparison to a reference photo.
 * --------------------------------------------------------------------------*/
...
#define MOON_DEBUG_GEOCENTRIC 0 /* 1 = force roll=0 AND libration=0 to show the
                                   north-up base for direct comparison */
#endif
 ...
 * Lunar texture.
 *
 * Primary path: decode the vendored Clementine equirectangular map
 * (MOON_EQUIRECT_JPG) into an RGB565 PSRAM buffer.
 ...
 * change in render_core and trigger a re-flip so flip toggles from the web UI
 * take effect live. -1 = "no texture decoded yet". */
...
{
    /* Free any previously decoded buffer so re-decode does not leak PSRAM. On
     * the first call s_tex_buf is NULL and this is a no-op. */
    if (s_tex_buf) {
    ...
    if (!s_tex_buf) {
        LOG_ERROR_F("[MoonSphere] PSRAM lunar texture alloc failed (%dx%d, %u bytes)\n",
                    w, h, (unsigned)bytes);
        return false;
    }
    s_tex_w = w;
 ...
 * Flip state comes from the live config (configStorage.getMoonFlipU/V):
 *  - flip_u mirrors columns (x -> w-1-x): east<->west longitude swap.
 *  - flip_v mirrors rows    (y -> h-1-y): north<->south latitude swap.
 * Both default OFF, so the base orientation is unchanged on defaults. The mirror
 * helpers are self-inverse, so toggling an axis from the web UI is just one more
 * mirror of that axis. */
...
{
    int flip_u = configStorage.getMoonFlipU() ? 1 : 0;
    int flip_v = configStorage.getMoonFlipV() ? 1 : 0;
...
static bool init_placeholder_texture(void)
{
    ...
    apply_texture_flips();
    LOG_WARNING_F("[MoonSphere] using procedural placeholder lunar texture %dx%d "
                  "(real texture decode unavailable)\n",
                  PLACEHOLDER_TEX_W, PLACEHOLDER_TEX_H);
    return true;
}

/* Decode the vendored Clementine equirectangular JPEG into the RGB565 PSRAM
 ...
 * right with 0 centered. Loaded as-is: top row = north, columns left-to-right
 * as stored. */
static bool init_real_texture(void)
{
    size_t len = (size_t)MOON_EQUIRECT_JPG_LEN;
    if (len == 0) {
        LOG_WARNING_F("[MoonSphere] embedded moon_equirect.jpg is empty\n");
        return false;
    }

    int w = 0, h = 0, ch = 0;
    /* Force 3 channels (RGB). req_comp=3 => stb returns 3 bytes per pixel (R,G,B). */
    unsigned char *img = stbi_load_from_memory(MOON_EQUIRECT_JPG,
                                               (int)len, &w, &h, &ch, 3);
    if (img == nullptr) {
        LOG_WARNING_F("[MoonSphere] stbi decode of moon_equirect.jpg failed: %s\n",
                      stbi_failure_reason() ? stbi_failure_reason() : "unknown");
        return false;
    }

    if (w <= 0 || h <= 0) {
        LOG_WARNING_F("[MoonSphere] moon_equirect.jpg bad dimensions %dx%d\n", w, h);
        stbi_image_free(img);
        return false;
    }
    ...
    stbi_image_free(img);
    apply_texture_flips();
    LOG_INFO_F("[MoonSphere] decoded real lunar texture %dx%d (src channels=%d) "
               "from moon_equirect.jpg\n", w, h, ch);
    return true;
}
 ...
 * inverse, so toggling an axis = mirroring that axis once. Guarded by the init
 * mutex. No-op when the flip state already matches (the common steady-state
 * path, zero cost). */
static void moon_sphere_reflip_if_changed(void)
{
    int want_u = configStorage.getMoonFlipU() ? 1 : 0;
    int want_v = configStorage.getMoonFlipV() ? 1 : 0;
...
/* Shaders compiled into the renderer:
 * orthographic projection + z-buffer + Gouraud (per-vertex Lambert/diffuse)
 * shading + bilinear texture sampling with power-of-two wrapping. GOURAUD (not
 * FLAT) so the sphere is lit per-vertex with normals interpolated across each
 * triangle, avoiding the faint facet grid FLAT shading produces. */
 ...
 * The caller owns both buffers and their lifetime. */
                                         ...
                                         uint16_t *color_buf, uint16_t *zbuf)
{
    ...
    /* Disk-scale-derived orthographic half-extent (NEW vs. NINA). With
     * s_disk_scale == 1.0 this equals NINA's fixed 1.08 (disk fills the frame).
     * Larger ORTHO_R -> a smaller disk within the frame, leaving the starfield/
     * glow background to fill out to the screen edge. Used for BOTH the renderer
     * setOrtho() AND the glow-halo R_disc geometry so the glow tracks the disk. */
    const float ORTHO_R = 1.08f / s_disk_scale;

    /* If the flip config changed since the texture was last decoded, re-apply the
     * flips now so web-UI flip toggles take effect live. No-op when unchanged. */
    moon_sphere_reflip_if_changed();

    /* ----- Background ----------------------------------------------------
     * Drawn DIRECTLY into color_buf BEFORE the sphere is rasterized, across the
     * WHOLE w*h buffer, so the background fills to the screen edge regardless of
     * disk size. The Renderer3D only writes the pixels the disc covers, so the
     * background shows through everywhere outside the lunar disc. Do NOT call
     ...
     * ORTHO_R, so the disc pixel radius is (1.0/ORTHO_R) * 0.5 * min(w,h). This
     * uses the SAME disk-scale-derived ORTHO_R as setOrtho below, so the glow
     * tracks the disk size. */
    if (bg_style & 2) {
    ...
    /* Orthographic box sized to a unit sphere (radius 1) using the disk-scale-
     * derived ORTHO_R half-extent. near/far bracket the sphere on the z axis. */
     ...
     * (terminator), diffuse high, specular off. Warm tint so the lit moon reads
     * warm-gray rather than neutral; the texture grayscale still shows through. */
     ...
     * tgx sphere + camera conventions (verified against the tgx sources):
     *  drawSphere() places a surface vertex at stack angle phi and sector angle
     *  theta at P = (sinPhi*cosTheta, cosPhi, sinPhi*sinTheta), so the POLE axis
     *  is +Y and longitude theta winds +X -> +Z -> -X -> -Z. Texture coords are
     *  u = theta/2pi, v = 0.5*cosPhi + 0.5. The equirectangular map is north-up
     *  (top row = lunar NORTH), so the texture maps NORTH to v=0 = geometry -Y.
     *
     *  The base orientation Ry(90)*Rx(180) maps EAST->+X (right), NORTH->+Y (up),
     *  SOUTH->-Y (bottom), sub-Earth -> +Z (toward the eye): the standard near
     *  side, not mirrored. Libration pre-rotates the body so the sub-Earth point
     *  lands at -X before the base rotation; Rz(roll) applies the parallactic
     *  roll about the view (+Z) axis.
     *
     *  Net vertex transform (innermost -> outermost):
     *      Ry(lib_lon) -> Rz(-lib_lat) -> Rx(180) -> Ry(90) -> Rz(roll)
     *  Lighting is computed in this same body frame and rotated by R; the lit
     *  fraction is invariant under R, so this only re-orients the lit disc on
     *  screen without touching the phase. MOON_DEBUG_GEOCENTRIC zeroes libration
     *  + roll to expose the base.
     */
    ...
    /* North-up handling: configStorage.getMoonNorthUp() == 1 forces the disc
     * upright/north-up (the geocentric north-up convention) by IGNORING the
     * parallactic/topocentric roll (st->roll); libration, phase, and the live
     * roll-trim offset still apply. When == 0 use st->roll (the located /
     * parallactic sky orientation). This mirrors NINA's moon_north_up flag.
     * (st->axis_P is the pre-parallactic axis position angle; the north-up
     * convention corresponds to no parallactic tilt, i.e. roll contribution 0.) */
    uint8_t north_up = configStorage.getMoonNorthUp();
    float roll_deg    = ((north_up == 1) ? 0.0f
                         : (float)(MOON_ROLL_SIGN) * st->roll * RAD2DEG)
                        + configStorage.getMoonRollOffset(); /* offset still applies */

    /* Live yaw/pitch offsets from config, added to the drag-supplied yaw/pitch.
     * Inside the #else branch so MOON_DEBUG_GEOCENTRIC (which zeroes yaw/pitch)
     * still wins when debug is enabled. */
    yaw_deg   += configStorage.getMoonYawOffset();
    pitch_deg += configStorage.getMoonPitchOffset();
#endif

    /* R_sky: the SKY orientation rotation (libration + base + parallactic roll).
     * Rotation-only; used UNCHANGED to place the sub-solar light into world space
     * so the sun stays fixed in the sky regardless of the user's drag yaw/pitch. */
    fMat4 R_sky;
    R_sky.setIdentity();
     ...
     * OUTERMOST (pitch then yaw appended after the sky-orientation calls). The
     * light is built from R_sky only, so the features rotate under a fixed
     * sub-solar point in TRUE_PHASE. */
    fMat4 M_rot = R_sky;
     ...
     * rotate it by R_sky into world space. setLightDirection() expects the
     * direction pointing TOWARD the light source; sun_w already points toward the
     * Sun, so pass it directly, no negation. */
    ...
    if (light_mode == MOON_LIGHT_EXPLORE) {
         ...
         * gentle view-aligned headlight (-Z) for mild shading with no night side. */
 ...
 * on success, nullptr on bad args. */
                                           ...
                                           moon_light_mode_t light_mode)
{
    ...
    if (!color_buf || !zbuf) {
        LOG_ERROR_F("[MoonSphere] PSRAM alloc failed (color=%p z=%p, %dx%d)\n",
...
/* Thin wrapper: the original sub-solar-lit, no-user-rotation render. */
