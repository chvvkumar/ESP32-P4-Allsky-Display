/*
 * PSRAM circular log buffer + chained vprintf hook. See system_log_capture.h.
 */
...
/* Stack scratch size for a single formatted log line. Longer lines are truncated
 * for capture only (the chained vprintf still prints the complete line). */
...
/* Append len bytes from src into the ring under the spinlock, overwriting the
 * oldest data when full. Handles wraparound at the physical end of buf. */
