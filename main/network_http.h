 ...
 *
 * NOT for: the large image download (streams into a pre-allocated fixed PSRAM
 * buffer with its own deadline/stall/watchdog policy -- owned by the image
 * pipeline) or OTA binary download. Those stay on their own paths.
 *
 * Thread-safety: an http_fetch_opts_t / http_fetch_conn_t pair is NOT safe to
 * share across tasks concurrently. Each task that wants keep-alive reuse owns
 * its own http_fetch_conn_t and passes only its own opts.
 */
...
typedef struct {
    ...
    bool use_tls_bundle;        /**< true -> esp_crt_bundle_attach (HTTPS cert validation);
                                  * false on an https URL skips verification (per-source
                                  * skip-verify toggle). */
    ...
    int *status_out;            /**< optional: written with the FINAL attempt's HTTP
                                  * status code whenever a status was received. Left
                                  * untouched on transport failure. Pre-set to 0 to
                                  * distinguish "no response" from a real status of 0. */
    const char *capture_header;  /**< optional: name of ONE response header to capture
                                  * as a raw string (case-insensitive match); NULL = off. */
    char *capture_header_out;    /**< caller buffer for the captured value (NUL-terminated,
                                  * truncated to fit); set to "" if the header was absent. */
 ...
 *
 * On success returns ESP_OK, and *out_body is a NUL-terminated PSRAM buffer the
 * caller must release with heap_caps_free(); *out_len is the body length
 * excluding the NUL. On failure returns an esp_err_t != ESP_OK and leaves
 * *out_body / *out_len untouched.
 *
 * @param opts  May be NULL to use all defaults (one-shot, no retry, 8s timeout,
 *              64KB cap, no TLS bundle).
 */
esp_err_t http_fetch_text(const char *url, const http_fetch_opts_t *opts,
                          char **out_body, size_t *out_len);

/**
 * Convenience wrapper: http_fetch_text() + cJSON_Parse(). Returns NULL on any
 * failure (transport, HTTP status, or JSON parse). Caller must cJSON_Delete().
 */
cJSON *http_fetch_json(const char *url, const http_fetch_opts_t *opts);

/**
 * Issue an HTTP HEAD and capture exactly one response header value. Used by the
 * HTTP-Date time-sync path. Returns ESP_OK if a 2xx/3xx status was seen and the
 * header was captured (or absent -> empty). @p opts may be NULL.
 */
esp_err_t http_fetch_head(const char *url, const http_fetch_opts_t *opts);

/** Create a persistent keep-alive slot. Returns NULL on allocation failure. */
