...
/* disk_scale is the fraction of the frame the lunar disc fills (1.0 = edge to
 * edge). Internally clamped to [0.2, 4.0]; the orthographic half-extent is
 * 1.08 / disk_scale so smaller values shrink the disc and let the background
 * fill the surrounding frame. */
 ...
 * view. Returns an RGB565 PSRAM buffer (caller frees) or NULL. */
uint16_t *moon_sphere_render_ex(int w, int h, const moon_state_t *st,
                                int nb_sectors, int nb_stacks, uint8_t bg_style,
                                float yaw_deg, float pitch_deg,
                                moon_light_mode_t light_mode, float disk_scale);
