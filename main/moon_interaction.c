...
{
    /* Read config OUTSIDE the spinlock (the getter takes its own lock) and sample
     * the release timestamp before entering the critical section, per the
     * "no esp_timer/app_config inside the lock" discipline. */
    uint8_t spin_mode = app_config_get_mSpinMode();
