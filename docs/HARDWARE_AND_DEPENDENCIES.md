# ESP32-P4 AllSky Display - Hardware & Dependencies
...
- **Special Features:**
  - PPA (Pixel Processing Accelerator) hardware engine
  - Hardware JPEG decoder support
  - USB-OTG for programming and debugging

**Why ESP32-P4?**
- **PPA Acceleration:** 10-50× faster image scaling/rotation vs. software
...
Two supported displays (compile-time selection via `CURRENT_SCREEN` define):

#### Option 1: 3.4" Round Display (800×800)
...
#### Option 2: 4.0" Round Display (720×720)
...
**Reason:** ESP-IDF 5.5 changed the enum type for `phy_clk_src`. The old constant name no longer exists.
...
```cpp
JPEGDEC jpeg;
if (jpeg.openRAM(imageBuffer, size, JPEGDrawCallback)) {
    int width = jpeg.getWidth();
    int height = jpeg.getHeight();
    jpeg.decode(0, 0, 0);  // Decode entire image
    jpeg.close();
}
```
...
**Source:** Built into ESP32 Arduino Core  
**No installation needed**

**Purpose:** HTTP server for configuration UI.
...
**Port:** Configurable (default 8080 in firmware)
...
wsServer.begin();
wsServer.loop();  // Call in main loop
```
...
ElegantOTA.begin(&webServer);  // Adds /update endpoint
...
**Purpose:** HTTP client for image downloads.
...
**Purpose:** NVS (Non-Volatile Storage) wrapper for configuration persistence.
...
### Display Selection (displays_config.h)
...
**After changing:**
1. Clean build: Delete `build/` folder
...
- **Libraries:** Arduino_GFX (patched), JPEGDEC, PubSubClient, WebSockets, ElegantOTA
